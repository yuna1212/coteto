...
date_pattern='(19|20)\d{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01]) (0[1-9]|1[012]):[0-5][0-9] [AP]M'
start_time=$(echo $start_time_line | perl -nle "print $& while m{$date_pattern}g")
echo $start_time_line

# 시작 시간에서 현재 시간 빼기
now_sec=$(date -j -f "%F %I:%M %p"  "$now_time" +%s)
start_sec=$(date -j -f "%F %I:%M %p"  "$start_time" +%s)

sec_diff=$(($now_sec-$start_sec))
min_diff=$(($sec_diff/60))
hour_diff=$(($min_diff/60))
min_diff=$(($min_diff%60))
echo 소요 시간: $hour_diff'시간' $min_diff'분'
...
if [ $hour_apprx -eq 0 ]
then
    consumed_time_format=$min_apprx'분'
elif [ $min_apprx -eq 0 ]
then
    consumed_time_format=$hour_apprx'시간'
else
    consumed_time_format=$hour_apprx'시간 '$min_apprx'분'
fi

echo "기록한 시간: $consumed_time_format" 
